...
public class RegisterCommandValidator : AbstractValidator<RegisterCommand>
{
    ...
    {
        RuleFor(p => p.Email).NotEmpty().NotNull().WithMessage("Mail Bilgisi Boş olamaz");

        RuleFor(p => p.Email).EmailAddress().WithMessage("Geçerli bir Email Adresi Giriniz");

        RuleFor(p => p.UserName).NotEmpty().NotNull().WithMessage("Kullanıcı Adı Bilgisi Boş olamaz");
        RuleFor(p => p.UserName).MinimumLength(5).WithMessage("Kullanıcı Adı En Az 3 Karakter Olmalıdır");
